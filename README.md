# Evidencia-1---ETL-entendimiento-datos-FEMSA---Isabela-Incer A00842223
Reporte de operaciones Spin Credit by OXXO



# Spin Credit : Análisis y reporte de operaciones 
## Descripción del proyecto: 
### Este proyecto evidencia el desarrollo del proyecto de analítica para FEMSA S.A., mediante el cual proponemos un modelo de analítica que permita optimizar el reporte de operaciones superiores a 188,282.22 MXN, a fin de cumplir con las reglas establecidas por el LFPIORI contra el lavado de activos y las operaciones financieras ilícitas o de dudosa procedencia. 
### Spin by OXXO es uno de los servicios de crédito digitales de FEMSA que surgió con el objetivo de brindar estos servicios a la población que no cuenta con historial bancario o acceso a bancos. Spin Credit es un intento de expandirse al negocio de los créditos, pero para ello necesita cumplir con varias regulaciones, cuyo objetivo en este proyecto es facilitar y garantizar su cumplimiento de la manera más eficiente. 


## Contenido del repositorio
- Entendimiento y contexto de FEMSA
- Limpieza de datos en Python
- Guía de SAT y UIT
- Seguridad, Conectividad y Validación
- Modelado de datos


